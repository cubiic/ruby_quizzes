...
# def solution(a)
#
# that, given a description of the turtle's walk in array A, returns the number of the first move in
# which the turtle touches its previous path, or 0 if no such situation occurs.
#
# For example, given array A as defined above, the function should return 7, because the turtle
...
def solution(a)
  move = 2
  touches = false
  while !touches && move < a.length do
    move += 1
    touches = a[move-1] <= a[move-3] && a[move-2] <= a[move]
  end
  touches ? move+1 : 0
end
